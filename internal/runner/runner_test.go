...
func TestRunner_RealisticRequestSimulation(t *testing.T) {
	...
	testRunner.metrics.RequestsPerSecond = 	int(testRunner.metrics.Requests) / int(duration.Seconds())
	...
	assert.Equal(t, 100, int(testRunner.metrics.Requests), "Expected Requests to be 100")
	assert.Equal(t, 50, int(testRunner.metrics.FailedRequests), "Expected FailedRequests to be 50")
	assert.InDelta(t, 33.33, testRunner.metrics.ErrorRate, 0.1, "Expected ErrorRate to be 33.33%")
	assert.InDelta(t, 10, testRunner.metrics.RequestsPerSecond, 0.1, "Expected RequestsPerSecond to be ~10")
